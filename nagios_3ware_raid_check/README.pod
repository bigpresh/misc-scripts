...
By default, this plugin expects to find the tw_cli binary in your $PATH.  If you
can run `sudo tw_cli` yourself and get output, it should work.  However, if you
need to, you can provide the path to the tw_cli binary using the `--tw_cli`
option, e.g.:
...
binary/script named tw_cli which appeared in some other directory earlier in
your $PATH.
