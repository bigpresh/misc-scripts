...
Update Feb 2024: I was having problems with `check_ide_smart` on a drive
in an external enclosure, where running it directly would work fine, but
when run via Nagios it would fail with `CRITICAL - SMART_CMD_ENABLE`.
...
package version `2.3.3-5+deb12u2`.
